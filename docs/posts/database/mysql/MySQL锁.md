...
tag:
  ...
  - 并发控制
  - 死锁
description: MySQL 表锁、行锁、意向锁、记录锁和隔离级别下的锁行为。
---
# MySQL锁
...
- 锁是MySQL在服务器层和存储引擎层的的并发控制。![mysql服务器逻辑架构](/images/legacy/mysql-locks/mysql服务器逻辑架构.jpg)
...
![锁](/images/legacy/mysql-locks/锁.png)
...
![锁模式兼容](/images/legacy/mysql-locks/锁模式兼容.jpg)

如果一个事务请求的锁模式与当前的锁兼容， InnoDB 就将请求的锁授予该事务； 反之， 如果两者不兼容，该事务就要等待锁释放

![mysql数据存储结构](/images/legacy/mysql-locks/mysql数据存储结构.jpg)
...
![聚集索引](/images/legacy/mysql-locks/聚集索引.jpg)


#### 辅助(普通)索引

![辅助索引](/images/legacy/mysql-locks/辅助索引.jpg)
...
![记录锁](/images/legacy/mysql-locks/记录锁.jpg)
...
![InnoDB不同隔离级别下锁比较1](/images/legacy/mysql-locks/InnoDB不同隔离级别下锁比较1.jpg)

![InnoDB不同隔离级别下锁比较2](/images/legacy/mysql-locks/InnoDB不同隔离级别下锁比较2.jpg)
