...
tag:
  ...
  - 多线程
  - 并发
description: Java 内存模型、volatile、happens-before 和指令重排学习笔记。
---
## 什么是JMM模型
...
线程，工作内存，主内存工作交互图（基于JMM规范）：
    ...
    ![0](/images/legacy/concurrent-java-memory-model/JMM规范.png)
...
模型如下图所示
    ...
    ![0](/images/legacy/concurrent-java-memory-model/工作内存模型.png)

#### Java内存模型与硬件内存架构的关系

通过对前面的硬件内存架构、Java内存模型以及Java多线程的实现原理的了解，我们应该已经意识到，多线程的执行最终都会映射到硬件处理器上进行执行，但Java内存模型和硬件内存架构并不完全一致。对于硬件内存来说只有寄存器、缓存内存、主内存的概念，并没有工作内存(线程私有数据区域)和主内存(堆内存)之分，也就是说Java内存模型对内存的划分对硬件内存并没有任何影响，因为JMM只是一种抽象的概念，是一组规则，并不实际存在，不管是工作内存的数据还是主内存的数据，对于计算机硬件来说都会存储在计算机主内存中，当然也有可能存储到CPU缓存或者寄存器中，因此总体上来说，Java内存模型和计算机硬件内存架构是一个相互交叉的关系，是一种抽象概念划分与真实物理硬件的交叉。(注意对于Java内存区域划分也是同样的道理)

    ![0](/images/legacy/concurrent-java-memory-model/java内存模型与硬件内存架构关系.png)
...
如以下示例图所示案例：
    ...
    ![0](https://note.youdao.com/yws/public/resource/e59837f57323a12defbb62fa837b330d/xmlnote/694FED5FA46C4BB9B8EEA4E936606690/14416)
...
4. **load(载入)**:作用于工作内存的变量,把read操作从主内存传输到线程的工作内存中,以便随后的load动作使用
...
如果要把一个变量从主内存中复制到工作内存中，就需要按顺序地执行read和load操作，如果把变量从工作内存中同步到主内存中，就需要按顺序地执行store和write操作。但Java内存模型只要求上述操作必须按顺序执行，而没有保证必须是连续执行。
    ...
    ![0](/images/legacy/concurrent-java-memory-model/数据同步八大原子操作.png)

#### 同步规则分析

1. 不允许一个线程无原因地(没有发生过assign操作)把数据从工作内存同步回主内存中
...
下图为从源码到最终执行的指令序列示意图：
    ...
    ![0](/images/legacy/concurrent-java-memory-model/指令序列.png)
...
下面是保守策略下，volatile写插入内存屏障后生成的指令序列示意图
    ...
    ![0](/images/legacy/concurrent-java-memory-model/volatile写后指令序列.png)
...
下图是在保守策略下，volatile读插入内存屏障后生成的指令序列示意图
    ...
    ![0](/images/legacy/concurrent-java-memory-model/volatile读后指令序列.png)
...
针对readAndWrite()方法，编译器在生成字节码时可以做如下的优化。
    ...
    ![0](/images/legacy/concurrent-java-memory-model/编译优化.png)
...
前面保守策略下的volatile读和写，在X86处理器平台可以优化成如下图所示。前文提到过，X86处理器仅会对写-读操作做重排序。X86不会对读-读、读-写和写-写操作 做重排序，因此在X86处理器中会省略掉这3种操作类型对应的内存屏障。在X86中，JMM仅需 在volatile写后面插入一个StoreLoad屏障即可正确实现volatile写-读的内存语义。这意味着在 X86处理器中，volatile写的开销比volatile读的开销会大很多（因为执行StoreLoad屏障开销会比较大）。
    ...
    ![0](/images/legacy/concurrent-java-memory-model/volatile写.png)
