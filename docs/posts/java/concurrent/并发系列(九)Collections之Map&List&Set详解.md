---
title: 并发系列(九)：Collections 之 Map、List、Set 详解
order: 9
date: 2023-03-01
category:
  - Java
...
tag:
  ...
  - List
  - Set
description: Map、List、Set 常见集合结构和并发场景问题整理。
---
## HashMap
...
如下图所示
    ...
    ![0](/images/legacy/concurrent-collections/4136.png)
...
那么此时状态为：
    ...
    ![0](/images/legacy/concurrent-collections/4148.png)
...
此时状态为：
    ...
    ![0](/images/legacy/concurrent-collections/4153.png)
...
这时候的状态如图所示：
    ...
    ![0](/images/legacy/concurrent-collections/4156.png)
...
扩容前：
    ...
    ![0](/images/legacy/concurrent-collections/16193.png)

扩容后：

    ![0](/images/legacy/concurrent-collections/4117.png)

由于Jdk8引入了新的数据结构，所以put方法过程也有了一定改进，其过程如下图所示。

    ![0](/images/legacy/concurrent-collections/4171.png)
...
Java7 ConcurrentHashMap基于ReentrantLock实现分段锁
    ...
    ![0](/images/legacy/concurrent-collections/16139.png)

Java8中 ConcurrentHashMap基于分段锁+CAS保证线程安全，分段锁基于synchronized关键字实现；

    ![0](/images/legacy/concurrent-collections/16135.png)
...
- 迁移链表和红黑树的原理 是一样的, 在红黑树中, 我们记录了每个红黑树的first(这个节点不是hash最小的节点)和每个节点的next, 根据这两个元素, 我们可以访问红黑树所有的元素, 红黑树此时也是一个链表, 红黑树和链表迁移的过程一样。红黑树根据迁移后拆分成了hn和ln, 根据链表长度确定链表是红黑树结构还是退化为了链表。
...
4、写的时候不能并发写，需要对写操作进行加锁； 
    ...
    ![0](/images/legacy/concurrent-collections/16242.png)
